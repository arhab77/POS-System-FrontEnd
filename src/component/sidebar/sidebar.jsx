...
const Sidebar = () => {
    ...
    useEffect(() => {
        axiosDriver.get('http://localhost:3001/auth/me')
        .then((res) => {
            setUser(res.data)
        })
    },[])
            ...
            <Container fluid>
                {
                    role === 'admin' ? (
                        <Nav className="flex-column">
                            ...
                            <hr />
                            <Nav.Link className='sidebar-link'><Link className='none'to={`/home`}><FaHome /> Home</Link></Nav.Link>
                            ...
                            <Nav.Link className='sidebar-link'><Link className='none'to={`/report`}><FaChartBar /> Report</Link></Nav.Link>
