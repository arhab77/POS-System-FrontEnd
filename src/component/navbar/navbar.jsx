...
const Navbars = () => {
    ...
    useEffect(() => {
        axiosDriver.get('http://localhost:3001/api/categories').then((res) => {
            setdata(res.data);
        })
    },[search])
    ...
    function getDataBySearch(e){
        e.preventDefault();
        var url = `http://localhost:3001/api/product?q=${search}`;
    ...
    //     axiosDriver.get(`http://localhost:3001/api/product?category=${data}`)
    //     .then((res) => {
    //         setdata(res.data);
